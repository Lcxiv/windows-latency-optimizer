...
- **Monitor Dashboard:** `monitor/` — canonical live UI. 9 views (Heatmap, Timeline, Processes, Drivers, Audit, Network, History, Command Center, Verdict). Reads JSON snapshots from `scripts/monitor_collector.ps1` every 2s. Same file:// constraints as dashboard.
